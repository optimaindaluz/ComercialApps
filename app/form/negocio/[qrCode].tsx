...
export default function BusinessFormScreen() {
  ...
  const { data: business, isLoading } = trpc.publicForm.getBusinessByQr.useQuery(
    { qrCode: qrCode ?? "" },
    { enabled: !!qrCode }
  );
  ...
  const styles = StyleSheet.create({
    ...
    headerBadgeText: { fontSize: 12, fontWeight: "700", color: colors.primary, letterSpacing: 0.5 },
    headerIcon: { fontSize: 40, marginBottom: 12 },
    headerTitle: { fontSize: 24, fontWeight: "800", color: colors.foreground, textAlign: "center" },
    headerSubtitle: { fontSize: 14, color: colors.muted, marginTop: 8, textAlign: "center", lineHeight: 22 },
    divider: { height: 1, backgroundColor: colors.border, marginHorizontal: 24, marginBottom: 8 },
    form: { padding: 24 },
    sectionTitle: { fontSize: 13, fontWeight: "700", color: colors.muted, letterSpacing: 0.5, marginBottom: 16 },
      ...
      <ScrollView keyboardShouldPersistTaps="handled">
        <View style={styles.header}>
          <View style={styles.headerBadge}>
            <Text style={styles.headerBadgeText}>⚡ COMPARATIVA DE LUZ</Text>
          </View>
          <Text style={styles.headerTitle}>{business.name}</Text>
          <Text style={styles.headerSubtitle}>
            Rellena el formulario y nuestro comercial te encontrará la mejor tarifa eléctrica
          </Text>
        </View>
        ...
        <View style={styles.form}>
          ...
          <TextInput style={styles.input} placeholder="Tu nombre y apellidos" placeholderTextColor={colors.muted}
          ...
          <TextInput style={styles.input} placeholder="6XX XXX XXX" placeholderTextColor={colors.muted}
            value={phone} onChangeText={setPhone} keyboardType="phone-pad" />

          <Text style={styles.label}>Email <Text style={styles.required}>*</Text></Text>
          <TextInput style={styles.input} placeholder="tu@email.com" placeholderTextColor={colors.muted}
          ...
          <View style={styles.fileSection}>
            ...
            {files.map((file, index) => (
              <View key={index} style={styles.fileItem}>
                <Text style={styles.fileIcon}>{file.mimeType.includes("pdf") ? "📄" : "🖼️"}</Text>
          ...
          <View style={styles.checkboxSection}>
            <TouchableOpacity style={styles.checkboxRow} onPress={() => setPrivacyAccepted(!privacyAccepted)}>
              ...
              <Text style={styles.checkboxText}>
                He leído y acepto la <Text style={styles.checkboxLink}>Política de Privacidad</Text> y el tratamiento de mis datos personales. <Text style={styles.required}>*</Text>
              </Text>
            </TouchableOpacity>

            <TouchableOpacity style={styles.checkboxRow} onPress={() => setMarketingAccepted(!marketingAccepted)}>
              <View style={[styles.checkbox, marketingAccepted && styles.checkboxChecked]}>
                {marketingAccepted && <Text style={styles.checkboxMark}>✓</Text>}
              </View>
              <Text style={styles.checkboxText}>
                Acepto recibir comunicaciones comerciales sobre ofertas de energía y otros servicios relacionados.
              </Text>
            </TouchableOpacity>
          </View>
